...
class MetadataIndex(BaseModel):
    ...
    """ # noqa: E501
    key: StrictStr = Field(description="Metadata field key to index")
    type: StrictStr = Field(description="Type of the metadata value")
    __properties: ClassVar[List[str]] = ["key", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['string', 'integer', 'float', 'boolean', 'datetime']):
            raise ValueError("must be one of enum values ('string', 'integer', 'float', 'boolean', 'datetime')")
        return value
